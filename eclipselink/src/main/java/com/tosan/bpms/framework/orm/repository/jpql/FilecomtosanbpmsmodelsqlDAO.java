 ...
 */
public class FilecomtosanbpmsmodelsqlDAO {
  ...
  public static java.util.List save(EntityManager entityManager, java.util.List models) {
    ...
    if (fetch != null && fetch.size() > 0) {
      ...
      while (fetchIterator.hasNext()) {
        ...
        com.tosan.bpms.model.sql.File model = (com.tosan.bpms.model.sql.File) fetchIterator.next();
    ...
    while (iterator.hasNext()) {
      com.tosan.bpms.model.sql.File model = (com.tosan.bpms.model.sql.File) iterator.next();

      if (model != null) {
        String key = model.getId() + "";
        com.tosan.bpms.model.sql.File fetchModel =
            (com.tosan.bpms.model.sql.File) existMap.get(key);
  ...
  public static java.util.List getByFilter(
    ...
    com.tosan.bpms.model.sql.File model = null;
    if (isFetch == false) {
      fetch = "";
    }
    ...
    java.lang.String select = "SELECT File0";
    if (selectParam != null && !selectParam.equals("")) {
      select = selectParam;
    }
    if (!isExistRecord && !isDelete) {
      if (fetchType == FetchType.LAZY) {
        basicQuery = select + " FROM com.tosan.bpms.model.sql.File File0  WHERE 1=1 ";
      } else if (fetchType == FetchType.EAGER) {
        basicQuery = select + " FROM com.tosan.bpms.model.sql.File File0  WHERE 1=1 ";
      }
    } else if (isExistRecord) {
      basicQuery = "SELECT File0 FROM com.tosan.bpms.model.sql.File File0 WHERE 1=1 ";
    } else if (isDelete) {
      if (fetchType == FetchType.LAZY) {
        basicQuery = "DELETE FROM com.tosan.bpms.model.sql.File File0 WHERE 1=1 ";
      } else if (fetchType == FetchType.EAGER) {
        basicQuery = "DELETE LF4FLMETADATA1 File0 WHERE 1=1 ";
      }
    }
    ...
    if (modelsLength > 0) {
      while (modelsIterator.hasNext()) {
        model = (com.tosan.bpms.model.sql.File) modelsIterator.next();
        query = "";
        isOpenParentheses = true;

        if (model != null) {

          if (isExistRecord == true && model.getId() == null) {
            model.setId("");
            isChangeKey = true;
          }
          ...
          if (model.getId() != null) {
            ...
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " File0.l4610flmtdataid = ? ";
              counter++;
            } else {
              whereQuery = " File0.id = :File0id" + j + " ";
            }

            isOpenParentheses = false;
            whereValue = model.getId();
          ...
          if (!isExistRecord && true && model.getExtension() != null) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " File0.l4610extension = ? ";
              counter++;
            } else {
              whereQuery = " File0.extension = :File0extension" + j + " ";
            }

            isOpenParentheses = false;
            whereValue = model.getExtension();
            query +=
                (andCondition
                    + Facade.getStatement(
                        statement,
          ...
          if (!isExistRecord && true && model.getVersion() != 0) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " File0.VERSION = ? ";
              counter++;
            } else {
              whereQuery = " File0.version = :File0version" + j + " ";
            }
    ...
    if (!isDelete) {
      ...
      if (cast != null
          && result != null
          && result.size() > 0
          && (!select.trim().equals("File0") && select.indexOf("new") == -1)) {
