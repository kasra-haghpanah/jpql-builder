 ...
 */
public class LogBoxcomtosanbpmsmodelsqlDAO {
  ...
  public static java.util.List save(EntityManager entityManager, java.util.List models) {
    ...
    if (fetch != null && fetch.size() > 0) {
      ...
      while (fetchIterator.hasNext()) {
        ...
        com.tosan.bpms.model.sql.LogBox model =
            (com.tosan.bpms.model.sql.LogBox) fetchIterator.next();
    ...
    while (iterator.hasNext()) {
      com.tosan.bpms.model.sql.LogBox model = (com.tosan.bpms.model.sql.LogBox) iterator.next();

      if (model != null) {
        String key = model.getId() + "";
        com.tosan.bpms.model.sql.LogBox fetchModel =
            (com.tosan.bpms.model.sql.LogBox) existMap.get(key);
  ...
  public static java.util.List getByFilter(
    ...
    com.tosan.bpms.model.sql.LogBox model = null;
    if (isFetch == false) {
      fetch = "";
    }
    ...
    java.lang.String select = "SELECT LogBox0";
    if (selectParam != null && !selectParam.equals("")) {
      select = selectParam;
    }
    if (!isExistRecord && !isDelete) {
      if (fetchType == FetchType.LAZY) {
        basicQuery = select + " FROM com.tosan.bpms.model.sql.LogBox LogBox0  WHERE 1=1 ";
      } else if (fetchType == FetchType.EAGER) {
        basicQuery = select + " FROM com.tosan.bpms.model.sql.LogBox LogBox0  WHERE 1=1 ";
      }
    } else if (isExistRecord) {
      basicQuery = "SELECT LogBox0 FROM com.tosan.bpms.model.sql.LogBox LogBox0 WHERE 1=1 ";
    } else if (isDelete) {
      if (fetchType == FetchType.LAZY) {
        basicQuery = "DELETE FROM com.tosan.bpms.model.sql.LogBox LogBox0 WHERE 1=1 ";
      } else if (fetchType == FetchType.EAGER) {
        basicQuery = "DELETE LogBox LogBox0 WHERE 1=1 ";
      }
    }
    ...
    if (modelsLength > 0) {
      while (modelsIterator.hasNext()) {
        model = (com.tosan.bpms.model.sql.LogBox) modelsIterator.next();
        query = "";
        ...
        if (model != null) {
          ...
          if (model.getId() != 0) {
            ...
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " LogBox0.id = ? ";
              counter++;
            } else {
              whereQuery = " LogBox0.id = :LogBox0id" + j + " ";
            }
          ...
          if (!isExistRecord && true && model.getFirstName() != null) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " LogBox0.firstName = ? ";
              counter++;
            } else {
              whereQuery = " LogBox0.firstName = :LogBox0firstName" + j + " ";
            }
          ...
          if (!isExistRecord && true && model.getLastName() != null) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " LogBox0.lastName = ? ";
              counter++;
            } else {
              whereQuery = " LogBox0.lastName = :LogBox0lastName" + j + " ";
            }
          ...
          if (!isExistRecord && true && model.getVersion() != 0) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " LogBox0.version = ? ";
              counter++;
            } else {
              whereQuery = " LogBox0.version = :LogBox0version" + j + " ";
            }
    ...
    if (!isDelete) {
      ...
      if (cast != null
          && result != null
          && result.size() > 0
          && (!select.trim().equals("LogBox0") && select.indexOf("new") == -1)) {
