 ...
 */
public class MeasurmentcomtosanbpmsmodelsqlDAO {
  ...
  public static java.util.List save(EntityManager entityManager, java.util.List models) {
    ...
    if (fetch != null && fetch.size() > 0) {
      ...
      while (fetchIterator.hasNext()) {
        ...
        com.tosan.bpms.model.sql.Measurment model =
            (com.tosan.bpms.model.sql.Measurment) fetchIterator.next();

        if (model != null) {

          String key = model.getMeasurementId() + "";
    ...
    while (iterator.hasNext()) {
      com.tosan.bpms.model.sql.Measurment model =
          (com.tosan.bpms.model.sql.Measurment) iterator.next();

      if (model != null) {
        String key = model.getMeasurementId() + "";
        com.tosan.bpms.model.sql.Measurment fetchModel =
            (com.tosan.bpms.model.sql.Measurment) existMap.get(key);
  ...
  public static java.util.List getByFilter(
    ...
    com.tosan.bpms.model.sql.Measurment model = null;
    if (isFetch == false) {
      fetch = "";
    }
    ...
    java.lang.String select = "SELECT Measurment0";
    if (selectParam != null && !selectParam.equals("")) {
      select = selectParam;
    }
    if (!isExistRecord && !isDelete) {
      if (fetchType == FetchType.LAZY) {
        basicQuery = select + " FROM com.tosan.bpms.model.sql.Measurment Measurment0  WHERE 1=1 ";
      } else if (fetchType == FetchType.EAGER) {
        basicQuery = select + " FROM com.tosan.bpms.model.sql.Measurment Measurment0  WHERE 1=1 ";
      }
    } else if (isExistRecord) {
      basicQuery =
          "SELECT Measurment0 FROM com.tosan.bpms.model.sql.Measurment Measurment0 WHERE 1=1 ";
    } else if (isDelete) {
      if (fetchType == FetchType.LAZY) {
        basicQuery = "DELETE FROM com.tosan.bpms.model.sql.Measurment Measurment0 WHERE 1=1 ";
      } else if (fetchType == FetchType.EAGER) {
        basicQuery = "DELETE Measurment Measurment0 WHERE 1=1 ";
      }
    }
    ...
    if (modelsLength > 0) {
      while (modelsIterator.hasNext()) {
        model = (com.tosan.bpms.model.sql.Measurment) modelsIterator.next();
        query = "";
        isOpenParentheses = true;

        if (model != null) {

          if (isExistRecord == true && model.getMeasurementId() == null) {
            model.setMeasurementId("");
            isChangeKey = true;
          }

          if (model.getMeasurementId() != null) {
            ...
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " Measurment0.id = ? ";
              counter++;
            } else {
              whereQuery = " Measurment0.measurementId = :Measurment0measurementId" + j + " ";
            }

            isOpenParentheses = false;
            whereValue = model.getMeasurementId();
            if (isExistRecord && isChangeKey) {
              model.setMeasurementId(null);
              isChangeKey = false;
            }
            ...
            query +=
                (andCondition
                    + Facade.getStatement(
                        statement,
                        "Measurment0.measurementId",
                        "Measurment0measurementId" + j,
                        mapVariable,
                        whereQuery,
                        whereValue,
                        isExistRecord,
          ...
          if (!isExistRecord && true && model.getName() != null) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " Measurment0.name = ? ";
              counter++;
            } else {
              whereQuery = " Measurment0.name = :Measurment0name" + j + " ";
            }

            isOpenParentheses = false;
            whereValue = model.getName();
            query +=
                (andCondition
                    + Facade.getStatement(
                        statement,
          ...
          if (!isExistRecord && true && model.getVersion() != 0) {
            ...
            query += " ";
            if (isDelete && fetchType == FetchType.EAGER) {
              whereQuery = " Measurment0.VERSION = ? ";
              counter++;
            } else {
              whereQuery = " Measurment0.version = :Measurment0version" + j + " ";
            }
    ...
    if (!isDelete) {
      ...
      if (cast != null
          && result != null
          && result.size() > 0
          && (!select.trim().equals("Measurment0") && select.indexOf("new") == -1)) {
